...
// import { FancyMultiSelect } from '@/components/common/form/multiSelect';
...
//   // const userRole = user?.role.map((role: { id: number }) => role.id);
...
//             // isSubmitting={isSubmitting || !form.formState.isValid}
...
export function UserForm() {
  ...
  const { mutate: updateUserRoles, isPending: isUpdatingRoles } =
    useUpdateResource<UserFormValues>(
      `user-group-assignments/${userId || createdUserData?.id}`,
      {
        onSuccess: () => {
